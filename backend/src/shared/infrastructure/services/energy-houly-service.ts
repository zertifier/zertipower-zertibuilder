    ...
    async getNewDatadisRegisters() {
        ...
        let query = `
            ...
            AND NOT EXISTS (
                ...
                WHERE e.info_dt = d.info_dt AND e.cups_id = d.cups_id
                AND e.info_dt >= ?
            );
        `
        ...
        for (const community of communities) {
            ...
            for (let start = 0; start < filteredCups.length; start += batchSize) {
                ...
                const query = `
                    ...
                    SET
                        kwh_in = CASE id ${cases.kwh_in} ELSE kwh_in END,
                        kwh_out = CASE id ${cases.kwh_out} ELSE kwh_out END,
                        production = CASE id ${cases.production} ELSE production END
