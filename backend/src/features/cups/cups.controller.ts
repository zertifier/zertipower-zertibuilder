...
export class CupsController {
  ...
  async getById(@Param('id') id: string) {
    ...
    return HttpResponse.success('cups fetched successfully').withData(this.mapData(data));
  }
  ...
  async datatables(@Body() body: any) {
    const data = await this.datatable.getData(body, `SELECT id,cups,provider_id,community_id,ubication,geolocalization,customer_id,created_at,updated_at
                  FROM cups`);
